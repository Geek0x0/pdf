...
// BenchmarkMemoryOptimizations 内存优化基准测试
// 用于对比优化前后的内存分配情况

func BenchmarkSortWithinBlock(b *testing.B) {
	texts := generateRandomTexts(1000, 100) // 1000个文本，分布在100行
...
func BenchmarkSmartTextRunsToPlain(b *testing.B) {
	texts := generateRandomTexts(5000, 200) // 5000个文本，分布在200行
...
func generateRandomTexts(count, lineCount int) []Text {
	...
	for i := 0; i < count; i++ {
		...
		texts[i] = Text{
			...
			X:        rand.Float64() * 500,
			Y:        float64(lineIdx*15) + rand.Float64()*2, // 行内微小偏移
...
// BenchmarkClusterTextBlocksMemOpt 测试聚类性能
...
// BenchmarkClusterTextBlocksSmall 小数据集聚类
...
// BenchmarkClusterTextBlocksLarge 大数据集聚类
